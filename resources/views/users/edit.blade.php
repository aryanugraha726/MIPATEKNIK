...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Ubah data akses dan role pengguna</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form action="{{ route('users.update', $user->user_id) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">ID Akun</label>
                <input type="text" name="user_id" value="{{ $user->user_id }}" class="w-full border-gray-300 rounded-lg shadow-sm bg-gray-100 text-gray-600 focus:border-blue-500 focus:ring-blue-500 cursor-not-allowed" readonly>
            </div>
            ...
            <div>
                ...
                <select name="id_karyawan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    @foreach($karyawans as $k)
                        <option value="{{ $k->id_karyawan }}" {{ (old('id_karyawan', $user->id_karyawan) == $k->id_karyawan) ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="username" value="{{ old('username', $user->username) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Ubah Password</label>
                <input type="password" name="password" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Biarkan kosong jika tidak ingin mengubah password">
            </div>
