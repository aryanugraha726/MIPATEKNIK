...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold">Edit Project #{{ $project->project_id }}</h1>
    ...
    <div class="bg-white rounded shadow-sm border border-gray-100 p-6">
        <form action="{{ route('projects.update', $project->project_id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_project" value="{{ old('nama_project', $project->nama_project) }}" class="shadow-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring focus:border-blue-300" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="prioritas" class="shadow-sm border rounded w-full py-2 px-3 text-gray-700 bg-white focus:outline-none focus:ring focus:border-blue-300" required>
                    <option value="TINGGI" {{ old('prioritas', $project->prioritas) == 'TINGGI' ? 'selected' : '' }}>TINGGI</option>
                    <option value="SEDANG" {{ old('prioritas', $project->prioritas) == 'SEDANG' ? 'selected' : '' }}>SEDANG</option>
                    <option value="RENDAH" {{ old('prioritas', $project->prioritas) == 'RENDAH' ? 'selected' : '' }}>RENDAH</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="date" name="target_project" value="{{ old('target_project', $project->target_project) }}" class="shadow-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring focus:border-blue-300" required>
                </div>
            </div>
